...
Amazon EC2 (Elastic Compute Cloud) is the backbone of AWS compute — it provides **virtual machines (VMs)** in the cloud that you can configure, launch, and scale as needed.

- **Elastic:** Scale capacity up or down as workloads change.
    
- **Compute:** You get CPU, memory, networking, and storage on demand.
    
- **Cloud:** You pay only for what you use — no upfront hardware.
...
### **Step 1 – Find an Amazon Linux 2 AMI**
...
🔹 **Concept:** Returns available AMIs owned by AWS.  
Amazon Linux 2 is commonly used because it’s lightweight, secure, and well-supported.
...
⚠️ **Security Tip:** Restrict `--cidr` to your IP for security, not `0.0.0.0/0`.
...
### **Step 5 – List and Monitor Instances**
...
🔹 **Concept:**  
Displays instance details — ID, state, name, and public IP.  
...
Attaching IAM roles lets your EC2 access AWS resources securely — e.g., S3, Glue, DynamoDB — **without storing credentials**.
...
🔹 **Concept:** Defines **who can assume the role** — here, EC2.
...
🔹 **Concept:** Attaches AWS-managed S3 policy to the role.
...
🔹 **Concept:**  
An **Instance Profile** is a container for the IAM Role — EC2 uses it to automatically fetch temporary credentials.
...
✅ EC2 can now call AWS APIs securely (e.g., `aws s3 ls`) without running `aws configure`.
...
|---|---|---|
|**Custom ETL**|Glue/Lambda lacks dependencies (e.g., TensorFlow, PyArrow).|S3 → EC2 process → Upload back to S3|
|**Long-running Jobs**|Heavy data prep or model training.|Airflow triggers EC2 job via SSH|
|**Hybrid Architecture**|Combine EC2 with Glue, Redshift, or Grafana.|S3 → EC2 → S3 (clean data) → Glue crawler|
...
- **Airflow DAG** → using SSH or EC2Operator.
    
- **AWS Step Functions** → start, process, stop instance.
...
Forward port `80 → 3000` and secure with Let’s Encrypt SSL for production.
...
1. Restrict SSH to your IP — avoid `0.0.0.0/0`.
...
|**Spot instances**|Up to 90% cheaper — great for batch ETL.|
